...
async function run() {
  console.log('=== Step 1: DDL Migration for blog_posts table ===');
  ...
  if (process.env.DATABASE_URL) {
    try {
      ...
      const sqlPath = path.join(__dirname, '..', 'supabase', 'migrations', '030_create_blog_posts.sql');
    ...
    console.log('NOTE: DATABASE_URL not set in environment.');
    console.log('If blog_posts table is not created yet, please run supabase/migrations/030_create_blog_posts.sql in the Supabase Dashboard SQL Editor.');
  }

  // Check if blog_posts table exists
  const { data: testSelect, error: selectErr } = await supabase.from('blog_posts').select('id').limit(1);
  if (selectErr) {
    console.error('\n❌ blog_posts table is not yet created or accessible via REST API:', selectErr.message);
    console.log('Please execute supabase/migrations/030_create_blog_posts.sql in your Supabase SQL Editor.');
    process.exit(1);
  }
  console.log('\n✅ blog_posts table confirmed present and accessible!');

  console.log('\n=== Step 2: Migrating Posts from lib/blog.ts to blog_posts ===');
...
**[Find vertical AI tools on Parlexa →](/directory)**
    ...
    },
    {
      ...
      author: 'Parlexa Team',
      published_date: '2026-04-20T00:00:00Z',
      read_time_minutes: 4,
      status: 'published',
      source: 'manual',
      faqs: [],
...
Generalists like standard GPT-4 are excellent communicators but often hallucinate complex niche logic (e.g. medical compliance, legal discovery). Vertical AI, trained exactly on proprietary datasets, guarantees safety, deterministic logic, and enterprise compliance.
  ...
  for (const post of postsToMigrate) {
    ...
    if (error) {
      console.error(`❌ Failed to upsert post "${post.slug}":`, error.message);
    } else {
      console.log(`✅ Upserted post "${post.slug}" (ID: ${data[0].id})`);
    }
  }
