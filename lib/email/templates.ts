...
export const templates = {
  ...
  welcomeConsumer: (name: string) => baseLayout(`
    <h1>Welcome to the Parlexa Ecosystem, ${name}! ðŸ‘‹</h1>
  ...
  welcomeLister: (name: string) => baseLayout(`
    ...
    <h1>Welcome to the Global Vendor Network, ${name}! ðŸš€</h1>
  ...
  listingSubmitted: (toolName: string) => baseLayout(`
    <h1>Submssion Received: ${toolName} â³</h1>
  ...
  listingApproved: (toolName: string, liveUrl: string) => baseLayout(`
    ...
    <h1>Congratulations! ${toolName} is Live! ðŸŽ‰</h1>
  ...
  listingRejected: (toolName: string, reason: string) => baseLayout(`
    ...
    <h1>Update needed for ${toolName} ðŸ“</h1>
  ...
  reviewAlert: (toolName: string, rating: number, reviewer: string) => baseLayout(`
    <h1>New ${rating}-Star Review for ${toolName} â­</h1>
    ...
    <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 20px; border-radius: 12px; margin: 20px 0;">
      <div style="color: #fbbf24; font-size: 20px; margin-bottom: 8px;">${'â˜…'.repeat(Math.round(rating))}${ 'â˜†'.repeat(5 - Math.round(rating))}</div>
  ...
  replyAlert: (toolName: string) => baseLayout(`
    <h1>The ${toolName} team has responded! ðŸ’¬</h1>
  ...
  verificationStatus: (toolName: string, approved: boolean, reason?: string) => baseLayout(`
    ...
    <h1>${approved ? `âœ“ ${toolName} is now Verified!` : `Verification update for ${toolName}`}</h1>
    <p>${approved 
      ? `Your verification request has been approved. The blue âœ“ Verified badge has been added to your listing, signaling trust to global enterprise buyers.`
  ...
  featuredAlert: (toolName: string, expiring: boolean) => baseLayout(`
    ...
    <h1>${expiring ? `Your promotion for ${toolName} is expiring soon â³` : `Your featured slot for ${toolName} is ACTIVE! ðŸ”¥`}</h1>
    <p>${expiring 
  ...
  weeklyDigest: (name: string, tools: { name: string, category: string, slug: string }[], unsubscribeLink: string) => baseLayout(`
    <h1>Weekly AI Insights for ${name} ðŸ§ </h1>
    ...
    <div style="margin: 24px 0;">
      ${tools.map(t => `
        <div style="padding: 16px; border-bottom: 1px solid #f1f5f9;">
          ...
          <a href="https://parlexa.in/products/${t.slug}" style="font-size: 12px; color: ${ACCENT_COLOR}; font-weight: 600;">Learn More â†’</a>
        </div>
      `).join('')}
    </div>
  ...
  savedToolVerified: (toolName: string, slug: string, unsubscribeLink: string) => baseLayout(`
    <div class="pill" style="background: rgba(16, 185, 129, 0.1); color: #10b981;">âœ“ Verification Alert</div>
  ...
  monthlyNewsletter: (unsubscribeLink: string) => baseLayout(`
    <h1>Parlexa Global Monthly: The AI Evolution ðŸŒ</h1>
  ...
  claimApproved: (toolName: string) => baseLayout(`
    ...
    <h1>You are now the owner of ${toolName}! ðŸŽ‰</h1>
