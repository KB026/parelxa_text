...
export default async function BundleDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="min-h-screen bg-[#09090B] text-white font-sans selection:bg-[#12B886]/30 selection:text-white">
      {/* Background Decorative Element */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-96 bg-gradient-to-b from-[#12B886]/10 via-[#12B886]/5 to-transparent blur-3xl pointer-events-none" />
      ...
      <main className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-24">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-400 mb-8">
          ...
          <ChevronRight className="w-3 h-3 text-gray-600" />
          <span className="text-[#12B886] font-semibold">{bundle.name}</span>
        </div>

        {/* HERO SECTION */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-16">
          <div className="lg:col-span-8 space-y-6">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#12B886]/10 border border-[#12B886]/30 text-[#12B886] text-xs font-bold uppercase tracking-wider">
            ...
            <div className="pt-4 space-y-3">
              <h3 className="text-xs font-extrabold text-[#12B886] uppercase tracking-wider mb-3">
                Key Kit Advantages:
              </h3>
              {bundle.benefits.map((benefit, idx) => (
                <div key={idx} className="flex items-start gap-3 p-3.5 rounded-xl bg-[#121215] border border-white/5">
                  <div className="w-6 h-6 rounded-full bg-[#12B886]/10 text-[#12B886] flex items-center justify-center shrink-0 mt-0.5 font-bold text-xs">
                    {idx + 1}
                  </div>
                  <span className="text-sm font-medium text-gray-200 leading-snug">{benefit}</span>
                </div>
              ))}
              <p className="text-sm font-semibold text-[#12B886] pt-2">
          ...
          <div className="lg:col-span-4 flex flex-col items-center justify-center p-8 rounded-3xl bg-[#121215] border border-white/10 text-center space-y-6">
            ...
            <div className="w-full pt-4 border-t border-white/10 flex items-center justify-center gap-2 text-xs text-[#12B886] font-semibold">
              ...
              return (
                <React.Fragment key={tool.agent_id}>
                  <Link
                    ...
                    className="flex-1 flex flex-col items-center text-center p-4 rounded-xl bg-[#18181C] border border-white/5 relative group hover:border-[#12B886]/50 transition-all text-left"
                  >
                    <span className="absolute -top-3 w-6 h-6 rounded-full bg-[#12B886] text-black font-bold text-xs flex items-center justify-center">
                      {idx + 1}
                    </span>
                    <div className="w-10 h-10 mb-3 rounded-lg bg-[#09090B] p-1 border border-white/10 flex items-center justify-center mt-2">
                      {tool.logo_url ? (
                    ...
                    <h4 className="text-sm font-bold text-white mb-1 group-hover:text-[#12B886] transition-colors">{tool.name}</h4>
                    <span className="text-[11px] text-[#12B886] font-semibold">{tool.role_in_workflow}</span>
                  </Link>

                  {!isLast && (
                    <div className="flex items-center justify-center text-[#12B886] px-1 shrink-0">
            ...
            {bundle.tools_full.map((tool, idx) => {
              ...
              return (
                <div key={tool.agent_id} className="space-y-4">
                  <Link
                    href={`/products/${tool.slug}`}
                    className="flex items-center gap-4 p-4 rounded-xl bg-[#121215] border border-white/10 hover:border-[#12B886]/50 transition-all relative block"
                  >
                    <div className="w-7 h-7 rounded-full bg-[#12B886] text-black font-extrabold text-xs flex items-center justify-center shrink-0">
                      {idx + 1}
                    </div>

                    <div className="w-9 h-9 rounded-lg bg-[#18181C] p-1 border border-white/10 flex items-center justify-center shrink-0">
                      {tool.logo_url ? (
                        <img src={tool.logo_url} alt={tool.name} className="w-full h-full object-cover rounded" />
                      ) : (
                        <span className="font-bold text-[#12B886] text-sm">{tool.name.charAt(0)}</span>
                    ...
                    <div>
                      ...
                      <span className="text-xs text-[#12B886] font-medium">{tool.role_in_workflow}</span>
                    </div>
                  </Link>

                  {!isLast && (
                    <div className="flex justify-center text-[#12B886] py-1">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
          <div className="p-8 rounded-2xl bg-[#121215] border border-white/10 space-y-4">
            <div className="w-10 h-10 rounded-xl bg-[#12B886]/10 text-[#12B886] flex items-center justify-center">
          ...
          <div className="p-8 rounded-2xl bg-[#121215] border border-white/10 space-y-4">
            ...
            <div className="pt-2">
              <span className="text-xs text-[#12B886] font-bold uppercase tracking-wider">
                Whole Kit Setup
              </span>
            </div>
          </div>
        ...
        <section className="p-8 sm:p-12 rounded-2xl bg-[#121215] border border-white/10 space-y-6">
          <div className="text-center max-w-xl mx-auto mb-8">
            <h2 className="text-2xl font-bold text-white flex items-center justify-center gap-2">
              <HelpCircle className="w-6 h-6 text-[#12B886]" />
